  ...
  return useMemo(() => {
    ...
    if (!activeRDMP) {
      if (hasDraftRDMP) {
        tasks.push({
          ...
          impact: 'Adding data and creating samples needs an active RDMP.',
          steps: [
            'Go to the RDMPs tab',
            'Review your draft RDMP content',
            'Click "Activate" to make it the active RDMP (needs PI role)',
          ],
          learnMore: 'Activating an RDMP enables all project operations including adding data, creating samples, and metadata editing. This action needs PI role and will set the data management policies for your project.',
          actionPath: '/rdmps',
        ...
        tasks.push({
          ...
          impact: 'Adding data and creating samples is not available until an RDMP is active.',
          steps: [
            'Go to the RDMPs tab',
            'Click "Create RDMP"',
          ...
          learnMore: 'An RDMP (Research Data Management Plan) defines how data is organized in your project, including required metadata fields, access policies, and file naming conventions. You need an active RDMP before you can add data.',
          actionPath: '/rdmps',
    ...
    if (activeRDMP && storageRoots.length === 0) {
      tasks.push({
        ...
        reason: 'No storage location is configured for adding data.',
        impact: 'You cannot add data files until a storage root is set up.',
        steps: [
          'Go to Settings',
          'Add a storage root pointing to your data location',
          'Verify the path is accessible',
        ],
        learnMore: 'A storage root tells the system where to look for new data files. Once configured, files placed in this location can be added to your project and linked to samples.',
        actionPath: '/settings',
    ...
    if (pendingCount > 0) {
      tasks.push({
        ...
        title: `Review ${pendingCount} pending file${pendingCount > 1 ? 's' : ''}`,
        ...
        steps: [
          ...
          'Finish adding the file to link it',
        ],
        learnMore: 'Pending files are data files that have been detected but not yet linked to samples. Review each file to ensure it is correctly associated with the right sample and has proper metadata.',
        actionPath: '/inbox',
    ...
    if (incompleteSamples.length > 0) {
      ...
      if (highPriority.length > 0) {
        tasks.push({
          ...
          reason: `${highPriority.length} sample${highPriority.length > 1 ? 's have' : ' has'} 3 or more missing core fields.`,
          impact: 'Incomplete metadata may affect data discoverability.',
          steps: [
            'Go to the Metadata Table',
            'Click on a sample to view details',
            'Fill in the missing core fields',
            'Save your changes',
          ],
          learnMore: 'Core metadata fields help ensure your data is properly documented and can be found by collaborators. The specific fields required are defined in your project RDMP.',
          actionPath: '/',
      ...
      if (lowPriority.length > 0 && highPriority.length === 0) {
        tasks.push({
          ...
          reason: `${lowPriority.length} sample${lowPriority.length > 1 ? 's have' : ' has'} missing core fields.`,
          ...
          learnMore: 'Core metadata fields help ensure your data is properly documented. The specific fields required are defined in your project RDMP.',
          actionPath: '/',
          actionLabel: 'View Samples',
          entityType: 'sample',
        });
